...
from offline_learning.hypervolume import InnerHyperVolume
...
def check_dominated(obj_batch, obj):
    return (
        np.logical_and((obj_batch >= obj).all(axis=1), (obj_batch > obj).any(axis=1))
    ).any()
...
def update_ep(ep_objs_batch, new_objs):
    ...
    for i in range(len(ep_objs_batch)):
        ...
        if (new_objs >= ep_objs_batch[i]).all():
            dominated = True
        if (ep_objs_batch[i] >= new_objs - 1e-5).all() and (
            ep_objs_batch[i] > new_objs + 1e-5
        ).any():
            on_ep = False
        if not dominated:
    ...
    if on_ep:
        inserted = False
        for i in range(
            len(new_ep_objs_batch)
        ):  # gaurantee the new ep objs is still in order with first objective
...
def generate_weights_batch_dfs(
    i, obj_num, min_weight, max_weight, delta_weight, weight, weights_batch
):
    if i == obj_num - 1:
    ...
    w = min_weight
    while (
        w < max_weight + 0.5 * delta_weight
        and np.sum(weight[0:i]) + w < 1.0 + 0.5 * delta_weight
    ):
        weight.append(w)
        generate_weights_batch_dfs(
            i + 1, obj_num, min_weight, max_weight, delta_weight, weight, weights_batch
        )
        weight = weight[0:i]
        w += delta_weight
