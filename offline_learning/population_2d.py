...
from offline_learning.util import get_ep_indices
...
def predict_hyperbolic(conf: RootConfig, opt_graph, optgraph_id, test_weights):
...
class Population:
    def __init__(self, conf: RootConfig):
        self.sample_batch: list[Sample] = []  # all samples in population
        self.pbuffer_num = conf.tuner.num_population_buffers
        self.pbuffer_size = conf.tuner.population_buffer_size
        self.dtheta = np.pi / 2.0 / self.pbuffer_num
        self.z_min = np.zeros(2)  # reference point
        self.pbuffers = None
    ...
    def compute_simple_hypervolume(self, batch: list[Sample], candidate: Sample):
        """Compute the hypervolume of the batch with the candidate sample added."""
        objs_batch = [sample.objs for sample in batch] + [candidate.objs]
        if len(objs_batch) < 2:
            return 0.0
        objs_batch.sort(key=lambda x: (x[0], x[1]))
        sum = 0
        for i in range(len(objs_batch) - 1):
            sum += (objs_batch[i + 1][0] - objs_batch[i][0]) * (
                objs_batch[i + 1][1] + objs_batch[i][1]
            )
        return sum

    def compute_simple_sparsity(self, batch: list[Sample], candidate: Sample):
        ...
        sum = 0
        for i in range(len(objs_batch) - 1):
            sum += (objs_batch[i + 1][0] - objs_batch[i][0]) ** 2
        return sum / (len(objs_batch) - 1)

    def result_guided_selection(
        self, conf: RootConfig, ep: EP
    ) -> tuple[list[Sample], list[ScalarizationFunction]]:
        """Select the next elite batch to train based on the performance of the models from the previous iteration.

        Args:
            conf: root config.
            ep: the whole population.
        Return: the next elite batch.
        """
        N = conf.tuner.num_tasks

        if len(self.sample_batch) < N:
            print(f"Not enough samples in the population, return all tasks")
            elite_batch = self.sample_batch
            scalarization_batch = []
            for sample in elite_batch:
                ...
                weights = np.array(
                    [
                        1 / obj if obj > 1e-3 else 1000
                        for obj in self.sample_batch[best_sample_id].objs
                    ]
                )
                scalarization_batch.append(
                    ScalarizationFunction(2, weights / np.sum(weights))
                )
        else:
            ...
            elite_batch.append(self.sample_batch[best_sample_id])
            ...
            for _ in range(N - 1):
                ...
                for i in range(len(self.sample_batch)):
                    if i in selected_ids:
                        continue
                    hv = self.compute_simple_hypervolume(
                        elite_batch, self.sample_batch[i]
                    )
                    sparsity = self.compute_simple_sparsity(
                        elite_batch, self.sample_batch[i]
                    )
                    metrics = hv - sparsity
                ...
                elite_batch.append(self.sample_batch[best_id])
                weights = np.array(
                    [
                        1 / obj if obj > 1e-3 else 1000
                        for obj in self.sample_batch[best_sample_id].objs
    ...
    def prediction_guided_selection(
        self, conf: RootConfig, iteration, ep, opt_graph, scalarization_template
    ):
        N = conf.tuner.num_tasks  # number of (sample, weight) to be selected
        num_weights = conf.tuner.num_weight_candidates
        ...
        for sample in self.sample_batch:
            ...
            if len(test_weights) > 0:
                results = predict_hyperbolic(
                    conf, opt_graph, sample.optgraph_id, test_weights
                )
                for i in range(len(test_weights)):
                    candidates.append(
                        {
        ...
        for _ in range(N):
            ...
            scalarization: ScalarizationFunction = deepcopy(scalarization_template)
    ...
    def random_selection(self, args, scalarization_template):
        ...
        for _ in range(args.num_tasks):
            ...
            weights = np.random.uniform(args.min_weight, args.max_weight, args.n_objs)
