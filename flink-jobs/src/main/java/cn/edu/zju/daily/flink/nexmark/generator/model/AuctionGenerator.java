 ...
 */
package cn.edu.zju.daily.flink.nexmark.generator.model;

import cn.edu.zju.daily.flink.nexmark.generator.NexmarkGeneratorConfig;
import cn.edu.zju.daily.flink.nexmark.model.Auction;
...
public class AuctionGenerator {
    /**
     * Keep the number of categories small so the example queries will find results even with a
    ...
    public static Auction nextAuction(
        ...
        long id = lastBase0AuctionId(config, eventId) + NexmarkGeneratorConfig.FIRST_AUCTION_ID;
        ...
        if (random.nextInt(config.getHotSellersRatio()) > 0) {
            ...
            seller =
                    (PersonGenerator.lastBase0PersonId(config, eventId) / HOT_SELLER_RATIO)
                            * HOT_SELLER_RATIO;
        } else {
        ...
        long category = NexmarkGeneratorConfig.FIRST_CATEGORY_ID + random.nextInt(NUM_CATEGORIES);
        ...
        String extra =
                StringsGenerator.nextExtra(random, currentSize, config.getAvgAuctionByteSize());
        return new Auction(
                id,
                name,
                desc,
    ...
    /**
     * Return the last valid auction id (ignoring FIRST_AUCTION_ID). Will be the current auction id
     * if due to generate an auction.
     */
    public static long lastBase0AuctionId(NexmarkGeneratorConfig config, long eventId) {
    ...
    public static long nextBase0AuctionId(
            long nextEventId, SplittableRandom random, NexmarkGeneratorConfig config) {
        ...
        long minAuction =
                Math.max(
                        lastBase0AuctionId(config, nextEventId) - config.getNumInFlightAuctions(),
                        0);
