...
import cn.edu.zju.daily.flink.nexmark.model.Bid;
...
public class BidGenerator {
    ...
    private static final String[] HOT_URLS =
            new String[] {
                getBaseUrl(random), getBaseUrl(random), getBaseUrl(random), getBaseUrl(random)
            };
    ...
    public static Bid nextBid(
            long eventId, SplittableRandom random, long timestamp, NexmarkGeneratorConfig config) {
        ...
        if (random.nextInt(config.getHotAuctionRatio()) > 0) {
            ...
            auction =
                    (AuctionGenerator.lastBase0AuctionId(config, eventId) / HOT_AUCTION_RATIO)
                            * HOT_AUCTION_RATIO;
        } else {
        ...
        if (random.nextInt(config.getHotBiddersRatio()) > 0) {
            // Choose the second person (so hot bidders and hot sellers don't collide) in the batch
            // of
            // last HOT_BIDDER_RATIO people.
            bidder =
                    (PersonGenerator.lastBase0PersonId(config, eventId) / HOT_BIDDER_RATIO)
                                    * HOT_BIDDER_RATIO
                            + 1;
        } else {
