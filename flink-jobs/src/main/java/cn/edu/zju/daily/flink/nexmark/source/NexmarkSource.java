 ...
 *
 * @deprecated Use {@link cn.edu.zju.daily.flink.nexmark.source.v2.NexmarkSource} instead.
 */
@Deprecated
public class NexmarkSource
        implements Source<
                        RowData,
    ...
    public NexmarkSource(NexmarkGeneratorConfig config, TypeInformation<RowData> outputType) {
        this.config = config;
    ...
    public SplitEnumerator<NexmarkSourceSplit, Collection<NexmarkSourceSplit>> createEnumerator(
        ...
        return new StaticSplitEnumerator(
                splitEnumeratorContext, getSplits(splitEnumeratorContext.currentParallelism()));
    }
    ...
    public SplitEnumerator<NexmarkSourceSplit, Collection<NexmarkSourceSplit>> restoreEnumerator(
            ...
            Collection<NexmarkSourceSplit> nexmarkSourceSplits)
            throws Exception {
        LOG.info("Restoring Nexmark Enumerator");
    ...
    @Override
    public SimpleVersionedSerializer<Collection<NexmarkSourceSplit>>
            getEnumeratorCheckpointSerializer() {
        return new SimpleSplitCollectionSerializer(SimpleSplitSerializer.INSTANCE);
    }

    @Override
    public SourceReader<RowData, NexmarkSourceSplit> createReader(
    ...
    List<NexmarkSourceSplit> getSplits(int parallelism) {
        List<NexmarkGeneratorConfig> subConfigs = config.split(parallelism);
        List<NexmarkSourceSplit> splits = new ArrayList<>(parallelism);
        for (NexmarkGeneratorConfig subConfig : subConfigs) {
    ...
    public static class StaticSplitEnumerator
        ...
        public void handleSplitRequest(int subtask, @Nullable String hostname) {
            ...
            final NexmarkSourceSplit split = splits.pollFirst();
            if (split != null) {
    ...
    public static class NexmarkSourceSplit implements SourceSplit, Serializable {
        ...
        @Getter private final NexmarkGeneratorConfig generatorConfig;
        @Getter @Setter private volatile long numEmittedSoFar;
        @Getter @Setter private long wallclockBaseTime;

        NexmarkSourceSplit(String id, NexmarkGeneratorConfig generatorConfig) {
            this.id = id;
            this.generatorConfig = generatorConfig;
            this.numEmittedSoFar = 0;
            this.wallclockBaseTime = -1;
        }
    ...
    public static class SimpleSplitSerializer
            implements SimpleVersionedSerializer<NexmarkSourceSplit> {
    ...
    public static class SimpleSplitCollectionSerializer
        ...
        public SimpleSplitCollectionSerializer(
                SimpleVersionedSerializer<NexmarkSourceSplit> splitSerializer) {
        ...
        public Collection<NexmarkSourceSplit> deserialize(int i, byte[] input) throws IOException {
            ...
            for (int remaining = numSplits; remaining > 0; remaining--) {
                ...
                bb.get(bytes);
                final NexmarkSourceSplit split =
                        splitSerializer.deserialize(splitSerializer.getVersion(), bytes);
                splits.add(split);
            }
            return splits;
        }
