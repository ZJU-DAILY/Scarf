...
public class NexmarkGenerator implements Iterator<NexmarkGenerator.NextEvent>, Serializable {
    ...
    public static class NextEvent implements Comparable<NextEvent> {
        ...
        public NextEvent(
                long wallclockTimestamp, long eventTimestamp, Event event, long watermark) {
     ...
     * #splitAtEventId}.
     */
    private NexmarkGeneratorConfig config;

    /**
     * Number of events generated by this generator. -- GETTER -- Gets Number of events generated by
     * this generator.
     */
    @Getter private long eventsCountSoFar;
    ...
    public NexmarkGenerator(
            NexmarkGeneratorConfig config, long eventsCountSoFar, long wallclockBaseTime) {
        checkNotNull(config);
        this.config = config;
    ...
    public NexmarkGenerator(NexmarkGeneratorConfig config) {
        this(config, 0, -1);
    }
     ...
     * #splitAtEventId}.
     */
    public NexmarkGeneratorConfig getCurrentConfig() {
        return config;
    }
     ...
     */
    public NexmarkGeneratorConfig splitAtEventId(long eventId) {
        long newMaxEvents = eventId - (config.firstEventId + config.firstEventNumber);
        NexmarkGeneratorConfig remainConfig =
                config.copyWith(
                        config.firstEventId,
    ...
    public NextEvent nextEvent() {
        if (wallclockBaseTime < 0) {
            throw new IllegalStateException(
                    "Cannot generate events before wallclockBaseTime is set");
        }
        ...
        // the event timestamp.
        long watermark =
                config.timestampForEvent(config.nextEventNumberForWatermark(eventsCountSoFar));
        // When, in wallclock time, we should emit the event.
        long wallclockTimestamp =
                wallclockBaseTime + (eventTimestamp - getCurrentConfig().getBaseTime());
            ...
            event =
                    new Event(
                            PersonGenerator.nextPerson(
                                    newEventId, random, adjustedEventTimestamp, config));
